...
public class Solution9 {
    /**
     * @author stern
     * @date 2020-02-17 12:21
     * @description
     *
     * 执行用时 : 16 ms, 在所有 Java 提交中击败了10.63%的用户
    ...
    */
    public static boolean isPalindrome_1(int x) {
        ...
        int left = count;
        int right = 0;
        while(x / (int)Math.pow(10, left) == x % (int)Math.pow(10, right + 1) / (int)Math.pow(10, right)) {
            // x -= x % (int)Math.pow(10, right + 1);
            x -= x / (int)Math.pow(10, left) * (int)Math.pow(10, left);
            if(left - right == 2 || left - right == 1) {
                return true;
            }
            left--;
            right++;
     ...
     * 内存消耗 : 45.1 MB, 在所有 Java 提交中击败了5.00%的用户
     *
    */
